...
void setup()
{
    ...
    app_controller->read_config(&app_controller->paj_cfg);
    ...
    app_controller->app_install(&cpm_app);
    // 优先显示屏幕 加快视觉上的开机时间
    //app_controller->main_process(&mpu.action_info);
    app_controller->main_process(&paj.action_info);

    /*** Init IMU as input device ***/
    lv_port_indev_init();
    paj.init(app_controller->sys_cfg.paj_order); // 初始化比较耗时
