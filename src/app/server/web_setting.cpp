...
#define CPM_SETTING "<form method=\"GET\" action=\"saveCpmConf\">"                                                                                      \
                    "<label class=\"input\"><span>ServerIP</span><input type=\"text\"name=\"IP\"value=\"%s\"></label>"                             \
                    "<label class=\"input\"><span>ServerPort</span><input type=\"text\"name=\"Port\"value=\"%s\"></label>" \
                    "</label><input class=\"btn\" type=\"submit\" name=\"submit\" value=\"保存\"></form>"
void init_page_header()
{
    ...
    webpage_header += F("<li><a href='/cpm_setting'>电脑性能监控</a></li>");
    webpage_header += F("</ul>");
}
...
void bili_setting()
{
    ...
    // app_controller->send_to(SERVER_APP_NAME, "cpm", APP_MESSAGE_READ_CFG,
    //                         NULL, NULL);
    // app_controller->send_to(SERVER_APP_NAME, "cpm", APP_MESSAGE_GET_PARAM,
    //                         (void *)"server_IP", bili_uid);
    // app_controller->send_to(SERVER_APP_NAME, "cpm", APP_MESSAGE_GET_PARAM,
    //                         (void *)"server_Port", updataInterval);
...
void cpm_setting()
{
    ...
    app_controller->send_to(SERVER_APP_NAME, "cpm", APP_MESSAGE_READ_CFG,
                            NULL, NULL);
    app_controller->send_to(SERVER_APP_NAME, "cpm", APP_MESSAGE_GET_PARAM,
                            (void *)"server_IP", server_IP);
    app_controller->send_to(SERVER_APP_NAME, "cpm", APP_MESSAGE_GET_PARAM,
                            (void *)"server_Port", server_Port);
    sprintf(buf,CPM_SETTING,server_IP,server_Port);
    webpage=buf;
    Send_HTML(webpage);
}
...
{
    Send_HTML(F("<h1>设置成功! 退出APP或者继续其他设置.</h1>"));
    //Serial.println(server.arg("bili_uid")+":"+server.arg("updataInterval").c_str());
    ...
    // app_controller->send_to(SERVER_APP_NAME, "cpm",
    //                         APP_MESSAGE_SET_PARAM,
    //                         (void *)"server_IP",
    //                         (void *)server.arg("bili_uid").c_str());
    // app_controller->send_to(SERVER_APP_NAME, "cpm",
    ...
    //                         (void *)server.arg("updataInterval").c_str());
    // 持久化数据
    app_controller->send_to(SERVER_APP_NAME, "cpm", APP_MESSAGE_WRITE_CFG,
                            NULL, NULL);
}
...
void saveCpmConf(void)
{
    Send_HTML(F("<h1>设置成功! 退出APP或者继续其他设置.</h1>"));
    Serial.println(server.arg("server_IP")+":"+server.arg("server_Port").c_str());
    app_controller->send_to(SERVER_APP_NAME, "cpm",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"server_IP",
                            (void *)server.arg("IP").c_str());
    app_controller->send_to(SERVER_APP_NAME, "cpm",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"server_Port",
                            (void *)server.arg("Port").c_str());
    // 持久化数据
    app_controller->send_to(SERVER_APP_NAME, "cpm", APP_MESSAGE_WRITE_CFG,
                            NULL, NULL);
